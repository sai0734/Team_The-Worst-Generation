...
CREATE TABLE IF NOT EXISTS tbl_allergy_ingredient(
    ingredient_no BIGINT AUTO_INCREMENT,
    ingredient_name VARCHAR(100) NOT NULL,
    PRIMARY KEY(ingredient_no)
);

CREATE TABLE IF NOT EXISTS tbl_baby_custom_allergy(
    custom_allergy_no BIGINT AUTO_INCREMENT,
    baby_no BIGINT NOT NULL,
    ingredient_name VARCHAR(100) NOT NULL,
    reg_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(custom_allergy_no),
    CONSTRAINT fk_custom_allergy_baby FOREIGN KEY(baby_no) REFERENCES tbl_baby_info(babyNo)
);

CREATE TABLE IF NOT EXISTS tbl_baby_allergy_check(
    check_no BIGINT AUTO_INCREMENT,
    baby_no BIGINT NOT NULL,
    image_file_name VARCHAR(500),
    ocr_raw_text VARCHAR(2000),
    detected_allergens VARCHAR(500),
    detected_custom VARCHAR(500),
    mixing_instruction  VARCHAR(1000),
    reg_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(check_no),
    CONSTRAINT fk_allergy_check_baby FOREIGN KEY(baby_no) REFERENCES tbl_baby_info(babyNo)
);

CREATE TABLE IF NOT EXISTS tbl_recipe_recommend(
    recommend_no BIGINT AUTO_INCREMENT,
    check_no BIGINT NOT NULL,
    product_type VARCHAR(20) NOT NULL,
    recommended_recipe VARCHAR(3000),
    reg_time DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(recommend_no),
    CONSTRAINT fk_recipe_recommend_check FOREIGN KEY(check_no) REFERENCES tbl_baby_allergy_check(check_no)
);

INSERT INTO tbl_allergy_ingredient (ingredient_name) VALUES
('알류(가금류)'),
('우유'),
('메밀'),
('땅콩'),
