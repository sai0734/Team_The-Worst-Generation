...
interface BabysitterMapComponentProps {
  sitters: BabysitterProfile[];
  center: { lat: number; lng: number };
}

const BabysitterMapComponent = ({ sitters, center }: BabysitterMapComponentProps) => {
  ...
  // center/sitters가 바뀔 때마다(내 주변 탭에서 필터·위치가 바뀔 때마다) 지도를 다시 그림
  useEffect(() => {
    let cancelled = false;
    ...
    const render = async () => {
      ...
      sitters.forEach((sitter) => {
        ...
        window.kakao.maps.event.addListener(marker, "click", () => {
          navigate(`/community/babysitter/${sitter.email}`);
        });
  ...
  }, [sitters, center.lat, center.lng]);

  return (
    <div
      ref={mapContainerRef}
      className="sitter-map-canvas"
      style={{ width: "100%", height: "100%", minHeight: 500 }}
    />
  );
};

export default BabysitterMapComponent;
